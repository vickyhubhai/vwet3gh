...
class Errors(Cog):
  ...
  async def on_command_error(self, ctx: Context, error):
    ...
    if isinstance(error, commands.CommandNotFound):
      ...
      hacker = discord.Embed(color=0x2f3136,description=f"❗ | {ctx.author.name} is on cooldown retry after {error.retry_after:.2f} second(s)", timestamp=ctx.message.created_at)
      hacker.set_author(name=f"{ctx.author}", icon_url=f"{ctx.author.avatar}")
      hacker.set_thumbnail(url =f"{ctx.author.avatar}")
      await ctx.reply(embed=hacker,delete_after=10)
    elif isinstance(error, commands.MaxConcurrencyReached):
      hacker = discord.Embed(color=0x2f3136,description=f"❗ | This Command is already going on, let it finish and retry after", timestamp=ctx.message.created_at)
      hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
      hacker.set_thumbnail(url =f"{ctx.author.avatar}")
      await ctx.reply(embed=hacker,delete_after=10)
      ctx.command.reset_cooldown(ctx)
      ...
      hacker = discord.Embed(color=0x2f3136,description=f"❗ | You lack `{fmt}` permission(s) to run `{ctx.command.name}` command!", timestamp=ctx.message.created_at)
      hacker.set_author(name=f"{ctx.author.name}", icon_url=f"{ctx.author.avatar}")
      hacker.set_thumbnail(url =f"{ctx.author.avatar}")

        ...
        for giveaway in giveaways:
            ...
            if int(time.time()) > data["end_time"] and not data["ended"]:
               ...
               # ctx = data["ctx"]
                ...
                result_embed2.set_footer(icon_url=self.bot.user.display_avatar.url, text=f"{len(winners)} winner | Ended" if len(winners) == 1 else f"{len(winners)} winners | Ended")
                ...
                view = View()
                view.add_item(btn)
                await giveaway_message.edit(content=f":tada: **Giveaway Ended** :tada:", embed=result_embed2, view=view)

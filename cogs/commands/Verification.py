...
class Verification(commands.Cog):
  ...
  async def verify_enable(self, ctx, channel: discord.TextChannel, role: discord.Role):
    ...
    if str(ctx.guild.id) in data:
      ...
      btn = Button(style=discord.ButtonStyle.grey, label="", emoji="<a:tick_vaibhavs_lithium:1166037808868237364>", custom_id="Verify")
      view = View()
      view.add_item(btn)
      embed = discord.Embed(title='Verification', description=f"To Access In *{ctx.guild.name}* Press the <a:tick_vaibhavs_lithium:1166037808868237364> Button Below", color=0x2f3136)

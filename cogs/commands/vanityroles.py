        ...
        with open("penny.json", "r") as f:
            ...
            if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
                if role.permissions.administrator or role.permissions.ban_members or role.permissions.kick_members:
                    ...
                    embed.set_author(name=f"Vanity Roles Config For {ctx.guild}", icon_url=ctx.author.display_avatar.url, url="https://discord.gg/MnPsa4ZFJ3")
                    embed.add_field(name="<a:arrow_arrow_vaibhavs_lithium:1094495285780152461>**__Vanity__**", value='Not Set' if vanity is None else vanity, inline=False)
                    embed.add_field(name="<a:arrow_arrow_vaibhavs_lithium:1094495285780152461>**__Role__**", value='Not Set' if role is None else role.mention, inline=False)
                    embed.add_field(name="<a:arrow_arrow_vaibhavs_lithium:1094495285780152461>**__Channel__**", value='Not Set' if channel is None else channel.mention, inline=False)
    ...
    async def on_presence_update(self, before, after):
      ...
      if str(before.guild.id) not in jnl:
        ...
        if vanity in str(after.activity).lower() and grole not in after.roles:
            ...
            embed=discord.Embed(color=0x030404, description=f"{after.mention} Thanks for Putting Vanity {vanity} in your status <3".title(), timestamp=datetime.datetime.utcnow())
            embed.set_author(name=after.name, icon_url=after.display_avatar.url)
            embed.set_footer(text="Thanks For Choosing Titanium.")
            ...
            embed=discord.Embed(color=0x030404, description=f"{after.mention} Removed Vanity {vanity} From His/Her Status <3".title(), timestamp=datetime.datetime.utcnow())
            embed.set_author(name=after.name, icon_url=after.display_avatar.url)
            embed.set_footer(text="Thanks For Choosing Titanium")
            await gchannel.send(embed=embed)

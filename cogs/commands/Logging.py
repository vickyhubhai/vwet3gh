...
def save(method, guild, channel):
  with open("loggingbyp.json","r") as f:
    idk = json.load(f)
  idk[str(guild)][method] = str(channel)
  with open("loggingbyp.json","w") as f:
...
Disables all the log in the server."""
    ...
    embed.set_footer(icon_url=ctx.bot.user.avatar, text="Thanks For Choosing 𝐅amtic 𝐏rime!")
  ...
  async def _msglog(self, ctx, channel: discord.TextChannel):
    if not await self._check(ctx):
      return await ctx.reply("<a:tick_vaibhavs_lithium:1166037808868237364> | Your top role should be above my top role.")
    save("msg",str(ctx.guild.id),str(ctx.channel.id))
    await ctx.send(f"<a:tick_vaibhavs_lithium:1166037808868237364> | Message log channel updated to {channel.mention}")
  ...
  async def __msglog(self, ctx, channel: discord.TextChannel):
    ...
    await ctx.send(f"<a:tick_vaibhavs_lithium:1166037808868237364> | Member log channel updated to {channel.mention}")
  ...
  async def ___msglog(self, ctx, channel: discord.TextChannel):
    ...
    await ctx.send(f"<a:tick_vaibhavs_lithium:1166037808868237364> | Server log channel updated to {channel.mention}")
  ...
  async def ___msglog_(self, ctx, channel: discord.TextChannel):
    ...
    await ctx.send(f"<a:tick_vaibhavs_lithium:1166037808868237364> | Channel log channel updated to {channel.mention}")
  ...
  async def __msglog__(self, ctx, channel: discord.TextChannel):
    ...
    await ctx.send(f"<a:tick_vaibhavs_lithium:1166037808868237364> | Role log channel updated to {channel.mention}")
  ...
  async def teccnobetamsglog(self, ctx, channel: discord.TextChannel):
    ...
    await ctx.send(f"<a:tick_vaibhavs_lithium:1166037808868237364> | Mod log channel updated to {channel.mention}")


  @commands.group(name="logall",invoke_without_command=True)
  async def _teknobeta(self, ctx):
    message = """`?logall enable <channel>`
Enables all the log in the given channel.
...
Disables all the log in the server."""
  ...
  async def logallenable(self,ctx,channel:discord.TextChannel):
    ...
    await ctx.send(f"<a:tick_vaibhavs_lithium:1166037808868237364> | All logs channel are updated to {channel.mention}")
  ...
  async def _dis(self,ctx):
    if not await self._check(ctx):
      return await ctx.send("<a:tick_vaibhavs_lithium:1166037808868237364> | Your top role should be above my top role.")
    with open("loggingbyp.json", "r") as f:
      fl = json.load(f)
    data = fl.get(str(ctx.guild.id))
    if data == {"msg": None, "member": None, "server": None, "channel": None, "role": None, "mod": None}:
      return await ctx.reply("<a:tick_vaibhavs_lithium:1166037808868237364> | No logging is enabled.")
    fl[str(ctx.guild.id)] = {"msg": None, "member": None, "server": None, "channel": None, "role": None, "mod": None}
    with open("loggingbyp.json", "w") as f:
      json.dump(fl,f,indent=4)
    await ctx.reply("<a:tick_vaibhavs_lithium:1166037808868237364> | Disabled all the logs for this server.")
